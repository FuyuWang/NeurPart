...
class MaestroEnvironment(object):
    ...
    def reset_dimension(self, model_name, model_defs, finish_reward=100, dim_size=6, n_action_steps=2, resource_size=2,
                        fitness='latency', dataflow="dla", num_pe=4096, l1_size=8000, l2_size=8000):
        random_file_name = random.randint(0, 2 ** 31)
        self.random_file_name = "{}".format(random_file_name)
        ...
        self.whole_eps_rewards = []
        self.min_eps_reward = self.model2min_eps_reward[model_name]
        self.best_reward = float('-inf')
        infos = {}
