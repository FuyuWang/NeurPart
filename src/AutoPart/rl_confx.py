...
class Agent():
    def __init__(self, dim_size, resource_size, n_action_steps, action_size, action_step_obs_size=1,lstm_hid_size=128, decay=0.95, is_gemm=False):
        ...
        self.actor = Actor(dim_size, resource_size, n_action_steps, action_size, lstm_hid_size).to(device)
    ...
    def act(self, state, infos, eps=0.0, temperature=1, pe_constraints={}):
        ...
        # print("dimensions: ", dimensions, action_status,actions, action_step, self.lstm_hid_value,temperature)
        action, log_prob, self.lstm_hid_value = self.actor(dimensions, action_status,actions, action_step, self.lstm_hid_value,temperature=temperature,
                                                           pe_constraints=pe_constraints)
        # print("p: ", p)
    ...
    def load_actor(self, chkpt):
        ...
        # print(chkpt["best_actor"])
        # self.actor.load_state_dict(chkpt["best_actor"])
...
class Actor(nn.Module):
    def __init__(self, dim_size, resource_size, n_action_steps, action_size, h_size=128, hidden_dim=10):
        ...
        self.fc21 = nn.Linear(h_size, h_size)
        self.fc22 = nn.Linear(h_size, h_size)
        self.fc23 = nn.Linear(h_size, h_size)
    ...
    def forward(self, dimension, action_status, action_val, action_step, lstm_hid,temperature=1, pe_constraints={}):
        ...
        action_space = pe_constraints['action_space']
        ktile_size_max = pe_constraints['ktile_size_max']
        dim = pe_constraints['dim']

        pe_space = action_space[0]
        ktile_space = action_space[1]
        pe_mask = torch.from_numpy(pe_space > remained_pes).to(dimension.device).float() * -1000000.
        # print(remained_pes, pe_mask)
        x1 = F.softmax(x1 / temperature + pe_mask, dim=1)
        # x1 = F.softmax(x1 / temperature, dim=1)
        policy1 = Categorical(x1)
        action1 = policy1.sample()
        log_prob1 = policy1.log_prob(action1)

        num_pe = pe_space[action1[0].item()]
        if dim[6] == 2:
            ClusterSz = 1
            k_dim = dim[1]
            pe_ktile_max = max(math.ceil(k_dim / (num_pe / ClusterSz)), ktile_space[0])
        else:
            ClusterSz = min(num_pe, 64)
            k_dim = dim[0]
            pe_ktile_max = max(math.ceil(k_dim / (num_pe / ClusterSz)), ktile_space[0])

        ktile_size_max = min(pe_ktile_max, ktile_size_max)
        ktile_mask = torch.from_numpy(ktile_space > ktile_size_max).to(x1.device).float() * -1000000.
        # ktile_mask = torch.from_numpy(ktile_space > pe_ktile_max).to(x1.device).float() * -1000000.
